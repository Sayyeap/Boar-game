class Player {
    constructor(scene, x, y, groundHeight) {
        ...
        this.groundY = scene.cameras.main.height - groundHeight + 110;
    ...
    setupInput() {
        this.scene.input.off('pointerdown'); // Очищаем старые обработчики
    ...
    startJump() {
        if (!this.isInputEnabled || this.isJumping || this.sprite.y < this.groundY) {
    ...
    attack(enemy) {
        ...
        try {
            ...
            // Наносим урон через 400 мс (середина анимации)
            ...
            this.sprite.once('animationcomplete', () => {
                ...
                this.isAttacking = false;
                if (this.sprite.y >= this.groundY && (!this.sprite.anims.currentAnim || this.sprite.anims.currentAnim.key !== 'jump_up' && this.sprite.anims.currentAnim.key !== 'jump_down')) {
    ...
    updateGroundY(newHeight) {
        this.groundY = newHeight - this.groundHeight + 110;
    ...
    enableInput() {
        ...
        this.sprite.setVelocityY(0);
        this.sprite.y = this.groundY;
        this.sprite.body.enable = true; // Включаем физику после фиксации y
        this.setupInput(); // Перерегистрируем обработчики ввода
        // Дополнительная фиксация позиции
        this.scene.time.delayedCall(100, () => {
            this.sprite.y = this.groundY;
            this.sprite.setVelocityY(0);
            console.log('Player: Input enabled, extra fix at y=' + this.sprite.y + ', groundY=' + this.groundY);
        });
